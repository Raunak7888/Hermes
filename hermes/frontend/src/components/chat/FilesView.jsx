  ...
  useEffect(() => {
    const fetchFile = async () => {
      ...
      try {
        ...
        setError("Maybe the sender deleted the file or it does not exist.");
      ...
      {loading ? (
        ...
        <div className={`flex my-2 ${isSent ? "justify-end" : "justify-start"}`}>
          <img
            ...
            className={`w-fit max-w-[30vw] aspect-auto object-cover ${imageClassName} rounded-xl cursor-pointer hover:opacity-90 transition-opacity duration-200 shadow-md`}
          />
        </div>
      ) : (
        <div
